...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Pressione o botom jogar para iniciar", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  if(/*(keyDown(control.up) ||keyDown(control.left)||keyDown(control.right) )*/ gameConfig.status==="start"&&GameStatus=="start"&&noseX!="") {
    world_start.play();
...
function manualControl(character){
  ...
  if(character.live){
    ...
    if(/*keyDown(control.right)*/noseX>300){
      character.velocity.x+=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(1);
    }
...
function jumping(character){
	if( (/*keyWentDown(control.up)*/noseY<200&&character.live) || (touchIsDown&&character.live) ){
